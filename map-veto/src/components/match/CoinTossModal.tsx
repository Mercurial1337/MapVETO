...
export function CoinTossModal({
    ...
    const [landedRotation, setLandedRotation] = useState(0);
    ...
    useEffect(() => {
        if (externalWinner) {
            ...
            setShowResult(true);
            setLandedRotation(externalWinner === 'team_a' ? 0 : 180);
        }
    }, [externalWinner]);
    ...
    const handleFlip = async (forcedWinner?: VetoActor) => {
        ...
        // Increment key to force CSS animation restart
        setFlipKey(prev => prev + 1);

        // Animation duration
        const animDuration = forcedWinner ? 1500 : 2500;

        // Start API call and wait for both animation and response
        const [winner] = await Promise.all([
            onFlip(forcedWinner),
            new Promise(resolve => setTimeout(resolve, animDuration))
        ]);

        if (winner) {
            // Set final rotation to show correct side
            // Team A = 0deg (front visible), Team B = 180deg (back visible)
            setLandedRotation(winner === 'team_a' ? 0 : 180);
            setResult(winner);
    ...
    const flipAnimationStyle = isFlipping ? {
        animation: 'coinFlipRotate 2.5s ease-in-out forwards',
    } : {
        transform: `rotateX(${landedRotation}deg)`,
        transition: 'transform 0.3s ease-out',
    };

    const bounceAnimationStyle = isFlipping ? {
        animation: 'coinBounce 2.5s ease-in-out',
    } : {};
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    {/* Coin - uses key to force remount and restart animation */}
                                    <div
                                        key={`coin-${flipKey}`}
